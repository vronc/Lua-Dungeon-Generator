...
local helpFunc = require("helpFunctions")
--[[
-- Testing Level
...
s = d:getStaircases()
for i=1,#d.staircases do
  if (not (s[i]==d.staircases[i])) then error("getStaircases test failed.") end
end
-- ## Tests for global functions ## --

if not getDist({0,3},{0,4}) == 5 then error("Level getDist test failed") end
